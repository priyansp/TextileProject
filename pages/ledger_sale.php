...
from (select v.vendor_name,SUM(CASE WHEN l.entry_type ='3' THEN l.amount WHEN l.entry_type='4' THEN -l.amount END)as total,
DATE_FORMAT(l.bill_for,'%b%y') as billed_for from ledger l right join vendors v on l.vendor_id=v.vendor_id 
where v.type=2 and (l.bill_for >='${six_month_earlier}' or l.bill_for IS NULL) 
...
$total_query="select v.vendor_name,l.balance,v.vendor_id from vendors v left join ledger l on v.vendor_id=l.vendor_id where (l.ledger_entry_id IS NULL or l.ledger_entry_id in (select MAX(ledger_entry_id) from ledger group by vendor_id)) and v.type=2 order by v.vendor_name;";
...
$(document).ready(function(){
    ...
    $('body').on("click","#download_button",function(){
        ...
        var filename="sale_ledger_"+moment().format('YYYY-MM-DD')+".csv";
